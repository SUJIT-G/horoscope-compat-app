// Netlify Function wrapper for the existing API (GET /api/horoscope/:zodiac, POST /api/compatibility)
// Requires: serverless-http (install with `npm i --save serverless-http`)
...
// Load horoscope.json from project root (Netlify functions run with project root as cwd)
let HOROSCOPE = {};
try {
  const horoscopePath = path.resolve(process.cwd(), 'horoscope.json');
  const data = fs.readFileSync(horoscopePath, 'utf8');
...
// Root info
app.get('/', (req, res) => {
  res.send('Horoscope + Compatibility Mini App API (Netlify Function) is running. Use /api/horoscope/:zodiac and /api/compatibility.');
});
